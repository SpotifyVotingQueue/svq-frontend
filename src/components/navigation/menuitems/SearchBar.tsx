...
import { APIContext, APIContextIF, QueueInformationContext, QueueInformationIF, SearchContext, SearchContextIF } from "../../../Providers";
...
export default function SearchBar() {
    ...
    async function onSearch(query: string) {
        console.log(query)
        api.track.searchTracks({ query: query, partyId: party.queueId as string }).then((res: TrackDto[]) => {
            search.setResult(res);
        });
    }

    return <div className="relative w-1/2 my-auto">
        <input type="text" className="bg-primary text-backgroundLight rounded-full h-8 pl-3 pr-2 pb-0.5" onChange={(e: React.ChangeEvent<HTMLInputElement>) => onSearch(e.target.value)} placeholder="Suche" onFocus={() => navigateSearch()} />

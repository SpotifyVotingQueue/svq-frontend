...
export default function Overview() {
	...
	let queueInformation: QueueInformationIF = useContext(
		QueueInformationContext
	);
	...
	const [playlists, setPlaylists] = useState(
		undefined as PlaylistDto[] | undefined
	);
	...
	useEffect(() => {
		async function initOverview() {
			...
			if (
				(!session.token || !session.clientSession) &&
				pathname === "/create" &&
				!debug &&
				!token
			) {
			...
			api.playlists
				.getPlaylists({ partyId: partyId })
				.then((res: PlaylistDto[]) => {
					setPlaylists(res);
				});
	...
	async function createNewQueue(
		...
		try {
			const res: PartyCreatedDto = await api.party.create({
				accesscode: token,
			});
			queueInformation.setQueueId!(res.joinCode);
			queueInformation.setJoinUrl!(
		...
		<>
			<div className="flex flex-col min-w-screen mb-4">
				<Transition appear show={qInfoOpen} as={Fragment}>
					<Dialog
						onClose={() => switch2Queue()}
						className="fixed top-0 left-0 z-20 bg-backgroundDark bg-opacity-90 min-h-screen min-w-screen"
					>
						<Transition.Child
							as={Fragment}
						...
						>
							<Dialog.Panel className="fixed top-1/3 left-0 ml-6 mr-6 mt-auto mb-auto rounded-2xl bg-secondary shadow-xl align-middle z-30">
								<Dialog.Title className="text-center text-primary font-roboto text-lg mt-4 font-bold">
									Teile die HipQueue
								</Dialog.Title>
								<Dialog.Description className="text-primaryDark font-roboto mr-4 ml-4 mt-4 text-center">
									Du kannst deine Gäste entweder per
									Beitritts-Code oder Link einladen
								</Dialog.Description>
								<div className="flex mr-4 ml-4 mt-4 justify-between">
									<p className="text-primary font-roboto w-0.3">
										Beitrittscode:
									</p>
									<input
										type="text"
										value={queueInformation.queueId}
										readOnly
										className="w-0.3 rounded-sm text-center text-primary font-bold text-lg bg-backgroundLight"
									/>
									<button
										className="w-0.3 text-primary text-sm font-roboto border-2 border-backgroundDark rounded-2xl bg-primaryDark pl-2 pr-2"
										onClick={() => copyID()}
									>
										{bTextID}
									</button>
								...
								<div className="flex mr-4 ml-4 mt-3 justify-between">
									...
									<input
										type="url"
										value={queueInformation.joinUrl}
										readOnly
										className="w-0.3 pl-1 pr-1 rounded-sm text-center text-lg text-primary bg-backgroundLight"
									/>
									<button
										className="w-0.3 text-primary text-sm  font-roboto border-2 border-backgroundDark rounded-2xl bg-primaryDark pl-2 pr-2"
										onClick={() => copyURL()}
								...
								<div className="flex mr-4 ml-4 mt-6 mb-4">
									<button
										className="w-full text-center text-2xl text-primary font-roboto border-2 border-backgroundDark rounded-2xl bg-primaryDark pl-2 pr-2"
										onClick={() => switch2Queue()}
									>
										Weiter
									</button>
								</div>
